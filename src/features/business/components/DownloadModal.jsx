...
const DownloadModal = ({ isOpen, onClose, onDownload }) => {
  ...
  const getDateRangeInfo = () => {
    ...
    switch (selectedDateRange) {
      case 'today':
        return `Datos de hoy (${today.toLocaleDateString('es-ES')})`
      case 'week':
        const weekStart = new Date(today)
        weekStart.setDate(today.getDate() - today.getDay())
        const weekEnd = new Date(weekStart)
        weekEnd.setDate(weekStart.getDate() + 6)
        return `Datos de esta semana (${weekStart.toLocaleDateString('es-ES')} - ${weekEnd.toLocaleDateString('es-ES')})`
      case 'month':
        const monthName = today.toLocaleDateString('es-ES', { month: 'long', year: 'numeric' })
        return `Datos de ${monthName}`
      case 'custom':
        return 'Rango personalizado seleccionado'
      default:
