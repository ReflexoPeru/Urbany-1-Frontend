...
export const NetworksProvider = ({ children }) => {
        ...
        () => networks.map((network) => (
            network.id === '2clics'
                ? {
                    ...
                    contactPhone: network.contactPhone || '+57 300 555 1020'
                }
                : network
        )),
        [networks]
    ...
    const networkSummaries = useMemo(() => (
        normalizedNetworks.map((network) => {
    ...
    ), [normalizedNetworks, statsByNetwork]);
    ...
    const createNetwork = useCallback((payload) => {
        const idValue = payload?.id ?? `network-${Date.now()}`;
        const newNetwork = {
            id: idValue,
            ...
            badgeText: payload?.badgeText || '0',
            connectionStatus: payload?.connectionStatus || 'draft',
            defaultCommission: payload?.defaultCommission ? Number(payload.defaultCommission) : null,
            sharePolicy: payload?.sharePolicy?.trim() || '',
            contactName: payload?.contactName?.trim() || '',
            contactEmail: payload?.contactEmail?.trim() || '',
            contactPhone: payload?.contactPhone?.trim() || '',
            syncFrequency: payload?.syncFrequency?.trim() || 'Manual',
            lastSync: new Date().toISOString()
        };

        setNetworks((previous) => [newNetwork, ...previous]);
        setSelectedNetworkId(idValue);
        return newNetwork;
    }, []);

    const updateNetwork = useCallback((networkId, updates) => {
        setNetworks((previous) => previous.map((network) => {
            ...
            return {
                ...network,
                ...updates,
                defaultCommission: updates?.defaultCommission !== undefined
                    ? (updates.defaultCommission === null || updates.defaultCommission === ''
                        ? null
                        : Number(updates.defaultCommission))
                    : network.defaultCommission,
                lastSync: updates?.lastSync ?? network.lastSync
            };
        }));
    }, []);
...
export const useNetworks = () => {
    ...
    if (!context) {
        throw new Error('useNetworks debe usarse dentro de NetworksProvider');
    }
    return context;
};
